...
"""
changes 2013-11-10:
    * the code is modified to represent the Extended Link Clustering algorithm
...
class HLC:
    ...
    def single_linkage(self, threshold=None, w=None):
        ...
        if w == None: # unweighted
            H = similarities_unweighted( self.adj, self.edges ) # min-heap ordered by 1-s
        else:
...
def similarities_unweighted(adj, edges):
    ...
    # modified code for the ELC algorithm
    min_heap = [] # elements are (1-sim,eij,ekl)
    for eij,ekl in combinations(edges,2): # all unordered pairs of edges
        edge_pair = swap( eij, ekl )
        i, j = eij
        k, l = ekl

        i_adj = dict( (n,adj[n] | set([n])) for n in [i, j, k, l])

        inc_ns_i,inc_ns_j, inc_ns_k, inc_ns_l = i_adj[i],i_adj[j],i_adj[k],i_adj[l] # inclusive neighbors
        ELS = 1.0 * (len(inc_ns_i&inc_ns_k) + len(inc_ns_i&inc_ns_l) + len(inc_ns_j&inc_ns_k) + len(inc_ns_j&inc_ns_l)) / (len(inc_ns_i|inc_ns_k) + len(inc_ns_i|inc_ns_l) + len(inc_ns_j|inc_ns_k) + len(inc_ns_j|inc_ns_l))
        heappush( min_heap, (1-ELS,edge_pair) )
